...
class Kernel_Layer(nn.Module):
    ...
    def _k_means(self, X):
        ...
        rand_points = torch.randint(0, X.shape[0], (self.hidden_dim,), dtype=torch.long)
        # randomly initialize centroids
        centroids = inputs[rand_points, :]
        # previous assignment
        prev = torch.randint(0, self.hidden_dim, (X.shape[0],), dtype=torch.long)
        iter = 0
        while iter < 1000:
            iter += 1
            # distance between each pair of vector
            cdist = torch.cdist(inputs, centroids, p=2)
            # cluster assignment
            new = cdist.argmin(dim=1)
            # stop if nothing changed
            if new.eq(prev).all() and iter > 1:
                break
            else:
                prev = new
            # new centroids
            for k in range(self.hidden_dim):
                centroids[k, :] = torch.mean(inputs[new.eq(k), :], dim=0)
        if iter == 1000:
    ...
    def forward(self, x):
        ...
        r = torch.empty(x.shape[0], self.prototypes.shape[0])
        dimension = (x.shape[0], self.prototypes.shape[0], x.shape[1])
        xi = x.unsqueeze(1).expand(dimension)
        xj = self.prototypes.unsqueeze(0).expand(dimension)
        #print(xi.shape)
        norms = ((xi - xj)**2).sum(-1)
        r = torch.exp(-1 * norms / (2 * (self.sigma ** 2)))
        return r
