...
class PCA():
    ...
    def _do_pca(self):
        ...
        n = self.X.shape[1]
        ones = np.ones((n,1))
        X_mean = (1/n)*self.X@ones
        XC = (self.X - X_mean@ones.T)
        XCt = XC.T
        u,_,_ = np.linalg.svd(XC@XCt)
...
class AE(nn.Module):
    ...
    def _network(self):
        ...
        self.Encoder = nn.Parameter(data=torch.empty((self.d_hidden_rep,self.n_features)))
        nn.init.kaiming_normal_(self.Encoder)
        self.Decoder = self.Encoder.transpose(0,1)
        self.w = self.Decoder
        
        # Note: here for the network without weights sharing 
        self.Encoder = nn.Parameter(data=torch.empty((self.d_hidden_rep,self.n_features)))
        self.Decoder = nn.Parameter(data=torch.empty((self.n_features,self.d_hidden_rep)))
        nn.init.kaiming_normal_(self.Encoder)
        nn.init.kaiming_normal_(self.Decoder)
        self.w = torch.empty_like(self.Encoder)
        ...
        self.net = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(256,self.d_hidden_rep),
            nn.Linear(self.d_hidden_rep,256),
            nn.ReLU(),
    ...
    def _forward(self, X):
        ...
        Encoded = torch.mm(self.Encoder,X)
        Decoded = torch.mm(self.Decoder,Encoded)
        return Decoded
        ...
        Decoded = self.net(X.transpose(0,1)).transpose(0,1)
        return Decoded
